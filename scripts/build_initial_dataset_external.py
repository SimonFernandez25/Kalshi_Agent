"""
Build initial dataset from 25 active markets using EXTERNAL TOOLS ONLY.

Purpose: Run the pipeline on active markets using tools that don't require
historical snapshot data:
- fred_macro_tool (FRED economic indicators)
- bls_labor_tool (BLS labor/CPI data)
- weather_probability_tool (WeatherAPI forecasts)
- sportsbook_implied_probability_tool (TheOddsAPI odds)

These tools read from external JSONL files collected independently,
so they work even for markets with no historical Kalshi price data.

Settings:
- 25 active markets (filtered by domain if needed)
- Threshold: 0.30 (low, to trigger more bets)
- Bet amount: $10 per market
- External tools only (no historical snapshot dependency)
"""
...
MAX_MARKETS = 25           # Number of active markets to process
MARKET_STATUS = "open"     # Active markets only
EXTERNAL_TOOLS = [
    'fred_macro_tool',
    'bls_labor_tool',
...
def build_external_formula(market_id: str, market_title: str, registry) -> FormulaSpec:
    """Build equal-weight formula with external tools only."""
    # Filter external tools to those that exist in registry
    available_external = [t for t in EXTERNAL_TOOLS if t in registry.tool_names]

    if not available_external:
        raise ValueError("No external tools available in registry")

    # For sports markets, always include weather and sportsbook odds
    domain = classify_market(market_id, market_title)
    if domain == 'sports':
        # Prefer sportsbook and weather for sports
        sports_tools = ['sportsbook_implied_probability_tool', 'weather_probability_tool']
        tool_list = [t for t in sports_tools if t in available_external]
        if not tool_list:
            tool_list = available_external[:2]  # Fallback
    elif domain == 'economics':
        # Prefer FRED and BLS for economics
        econ_tools = ['fred_macro_tool', 'bls_labor_tool']
        tool_list = [t for t in econ_tools if t in available_external]
        if not tool_list:
            tool_list = available_external[:2]
    else:
        # Use all available external tools for other domains
        tool_list = available_external
    ...
    return FormulaSpec(
        ...
        threshold=THRESHOLD,
        rationale=f'Equal-weight {n} external tools (no snapshot dependency)'
    )


def main():
    print("=" * 80)
    print("  INITIAL DATASET BUILD - EXTERNAL TOOLS ONLY")
    print("=" * 80)
    ...
    print(f"  External Tools  : {', '.join(EXTERNAL_TOOLS)}")
    print("=" * 80)
    print()
    ...
    available_external = [t for t in EXTERNAL_TOOLS if t in registry.tool_names]
    if not available_external:
        print("ERR No external tools found in registry!")
        print("Available tools:", registry.tool_names)
        return

    print(f"External tools available: {available_external}")
    print()

    # Fetch active markets
    print(f"[1/4] Fetching {MAX_MARKETS} active markets from Kalshi...")
    try:
    ...
    # Process each market
    print(f"[2/4] Processing markets with external tools...")
    print("-" * 80)
    ...
    for i, mkt in enumerate(markets, 1):
        ...
        # Build formula with external tools only
        try:
            formula = build_external_formula(market_id, title, registry)
        except ValueError as e:
            print(f"      ERR {e}")
            failed.append({'market_id': market_id, 'reason': str(e)})
            print()
            continue
        ...
        # Log to SQLite
        run_id = f"external_{i:03d}"
        try:
            # Determine outcome if available
            outcome = None
